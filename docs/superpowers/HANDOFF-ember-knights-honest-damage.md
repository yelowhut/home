...
- **Крит** = `×(1 + Get(CritDmgMUL))`. У игрока CritDmgMUL=0.5 → крит ×1.5 (НЕ ×2),
  ...
  `HIT:22 (crit 32)`.
- **Округление урона: игра УСЕКАЕТ дробь (floor), НЕ округляет** (подтверждено 2026-06-29:
  мод стабильно показывал +1 на атаках с дробью ≥0.5). Показ через `FmtDmg()` = `(int)`-каст
  (floor для ≥0). НЕ использовать `:F0` (банкирское округление к ближайшему). Крит:
  применено `floor(ATK×mul×critMul)` — порядок «крит→усечение» НЕ верифицирован отдельно.
